...
{  
	...
	double iter=1.0,Tnf;
	double x_min[Dim]={0,0,0,0,0,0,0},x_max[Dim]={XMAX,0.25,XMAX,0.09,0.27,0.35,0.16}; 
 
	char a_1[20]="p"; 
	double Tn=3e12;
	double Tni=1e5;
	double h=0.005,x[Dim]={0.5,0.1,0.8,0.04,0.04,0.1,0.01};
	index1 = 2;
	index2 = 3;
	...
	for(iter=1.0;iter<=Tnf;iter=iter+1)                                      
	{
		rk2(h,x,fvec,x_min,x_max,point);   
			if(fmod(iter,10000000000)==0)  //function of fmod is remainder 
	           printf("%e	%f\n",iter,iter*h);   
